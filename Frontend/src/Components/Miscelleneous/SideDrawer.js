import { Box,Button, Tooltip, Text, Menu, MenuButton, MenuList, MenuItem, MenuDivider, Drawer, DrawerOverlay, DrawerContent, DrawerHeader, DrawerBody, Input, useDisclosure, useToast, Spinner } from '@chakra-ui/react';
...
const logoutHandler =() => {
 ...
 const handleSearch = async() => {
  ...
  try {
     setLoading(true)
     const config = {
      headers : {    // we are supposed to send the jwt token, and we have made this route as "protected", so it takes some headers 
          ...
          <MenuList pl ={3}>
            ...
            {notifications.map((notif) => (
              <MenuItem key={notif._id} onClick={() => {
                ...
                {notif.chat.isGroupChat ? `New Message in ${notif.chat.chatame}` :
      ...
      <DrawerOverlay>
        <DrawerContent>
          ...
          <DrawerBody>
            ...
            {loading ? (<ChatLoading /> ): (
              // optional chaining
              searchResults?.map(user => (             // handlesearch se users jo aaye , unki chat access krre hain
                <UserListItem
                key ={user._id}
                user ={user}
